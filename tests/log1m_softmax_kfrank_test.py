import unittest
from log1msoftmax import log1m_softmax_kfrank as log1m_softmax
import torch
from mpmath import *

mp.prec=1000

class Testlog1m_softmax_kfrank(unittest.TestCase):
    ...
    def test_precision(self):
        ...
        res1 = log1m_softmax(a, -1)
        res2 = torch.stack([log1m_softmax(a[i], -1) for i in range(a.shape[0])])
        self.assertTrue((res1 == res2).all())

        a = torch.randn((10,2,500))
        res1 = log1m_softmax(a, -1)
        res2 = torch.stack([log1m_softmax(a[i,j], -1) for i in range(a.shape[0]) for j in range(a.shape[1])]).view(10,2,-1)
        self.assertTrue((res1 == res2).all())

        a = torch.randn((10,2,500))
        res1 = log1m_softmax(a, 1)
        res2 = torch.stack([log1m_softmax(a[i,:,j], -1) for i in range(a.shape[0]) for j in range(a.shape[2])]).view(10,500,2).swapaxes(1,2)
        self.assertTrue((res1 == res2).all())
